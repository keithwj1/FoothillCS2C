...
public:
    ...
    Value& operator[](const Key& key) {
        ...
        std::unique_ptr<Node>*& cur = insert(key, root_);
        cur = insert(key, root_);
        return cur->get()->value_;
    }
    Value& operatorgoodvalue(const Key& key) {
        // Try to find the node with the value we want:
        std::unique_ptr<Node>* cur;
        for (cur = &root_;
            cur->get() != nullptr;
    ...
    std::unique_ptr<Node>* & insert(const Key& key, std::unique_ptr<Node>& node) {
        ...
        std::unique_ptr<Node>* cur = &balance(node);
        return cur;
        //return cur;
    }
    Node* root() {
